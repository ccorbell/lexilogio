...
class DeckDatabase:
    ...
    def queryForDeckTerms(self, deck: Deck, 
                          category:str = None, 
                          binValues:list = None,
                          tags:list = None):
        self.ensureDeckTablesExist(deck)
...
"""
    ...
    def updateTermBins(self, deck: Deck, termList:list, isReversedDrill=False):
        ...
        UPDATE_BIN_SQL = f"UPDATE {DECK_TABLE_NAME} SET bin = ?, last_drill_time = ? WHERE pkey = ?;"
    ...
    def readDeckPreferences(self, deck:Deck):
        ...
        deck.prefs = {Deck.PREFSKEY_QUESTION_COUNT: int(qCount),
    ...
    ?, ?, ?
        ...
        cur = con.cursor()
        cur.execute(WRITE_SQL, [(qCount), (useSRintValue), (reversedDrillIntValue),])
        con.commit()
