...
class TextDrillRunner:
    ...
    def initialize(self, dataDir, 
        ...
        print(f"Loaded deck {deckName} from {databaseFileName}, which contains {len(self.deck.terms)} terms.")
        if len(self.deck.terms) == 0:
            print("NOTE: loxilogio needs terms to be added before a drill can be run from this deck")
    ...
    def prepare_and_run_drill(self):
        ...
        print("1 = not known, 2 = hard to recall, 3 = somewhat known, 4 = familiar, 5 = well known")
    ...
    def run_drill_response_input(self):
        ...
        while not rating >= 1 and rating < 5:
            ...
            try:
                ...
                print(f"invalid rating value \"{rating}\", please enter a value from 1 to 5")
                rating = 0
                continue
    ...
    def end_drill(self):
        ...
        if len(updatedTerms) > 0:
            self.database.updateTermBins(self.deck, updatedTerms, self.deck.isReversedDrill())
    ...
    def run_prefs(self):
        ...
        while not choice == 'x':
            ...
            choice = input("\nEnter letter of preference to change, or x to exit: ").strip().lower()
            if choice == 'a':
                ...
                yn = input(f"Set 'use spaced repetition' to {toggleSpaceRep}? (y/n): ").strip().lower()
                if yn.startswith('y'):
                    self.deck.prefs[Deck.PREFSKEY_SPACED_REPETITION] = toggleSpaceRep
                    self.database.writeDeckPreferences(self.deck)
            elif choice == 'c':
                toggleReversedPref = not reversedPref
                yn = input(f"Set 'reverse drill' to {toggleReversedPref}? (y/n): ").strip().lower()
                if yn.startswith('y'):
    ...
    def run_import(self):
        ...
        print("# category=NAME which will set the category to NAME for subsequent terms.")
            ...
            if termLine.startswith("#"):
                ...
                if procLine.startswith("category"):
                    ...
                    if None == runningCategory:
                        raise Exception(f"ERROR - category line should have the format # category=(value), found:\n{termLine}")
                continue
    ...
    def show_setup_menu(self):
        ...
        choice = input("(d) drill (i) import/add (c) categories (t) tags (x) exit: ").strip().lower()
        if choice == 'd':
            self.prepare_and_run_drill()
        elif choice == 'i':
            pass
...
def importAllGreekFiles():
    ...
    importFiles = [adjFile, advFile, conjFile, nounFile, prepFile, phraseFile, pronounFile, verbsFile]
